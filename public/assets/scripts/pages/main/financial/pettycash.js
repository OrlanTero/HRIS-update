...
function ViewRequest(id) {
    const popup = new Popup("pettycash/view_pettycash", {id: id}, {
    ...
    popup.Create().then(((pop) => {
        ...
        ListenToForm(form, function (data) {
            ...
            EditRecord(TARGET, {id, data: JSON.stringify(data)}).then((res) => {
                NewNotification({
                    title: res.code === 200 ? 'Success' : 'Failed',
                    message: res.code === 200 ? 'Successfully Updated Cash' : 'Task Failed to perform!'
                }, 3000, res.code === 200 ? NotificationType.SUCCESS : NotificationType.ERROR)
    ...
    popup.Create().then(((pop) => {
        ...
        ListenToForm(form, function (data) {
            AddRecord(TARGET, {data: JSON.stringify(data)}).then((res) => {
                NewNotification({
                    title: res.code === 200 ? 'Success' : 'Failed',
                    message: res.code === 200 ? 'Successfully Added Cash' : 'Task Failed to perform!'
                }, 3000, res.code === 200 ? NotificationType.SUCCESS : NotificationType.ERROR)
...
function ManageButtons() {
    ...
    const FLTER = new FilterContainer({}, { table: "pettycash", id: "pettycash_id", control: "PETTYCASH_CONTROL"});

    FLTER.Create().then(() => FLTER.Hide());

    FLTER.Load("PETTYCASH_TABLE_HEADER_TEXT", "PETTYCASH_TABLE_BODY_KEY")
